...
network_slug = 'InternetComputer'

# Fetch total supply for a given token
def fetch_ckbtc_total_supply(url):
    try:
        ...
        for line in lines:
            if line.startswith("ledger_total_supply"):
                supply_value = float(line.split()[1])
                ckbtc_supply = supply_value / 10**8
                return ckbtc_supply
        return None
...
def lambda_handler(event, context):
    ...
    ic_rpc_url = api_secret.get('RPC_INTERNETCOMPUTER')
