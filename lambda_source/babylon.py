...
network_slug = 'Babylon'

# Fetch total supply for a given token and block
def get_total_supply(token_slug, babylon_rpc_url):
    log.info(f"Fetching total supply for {token_slug}")

    response = requests.get(babylon_rpc_url)
    response.raise_for_status()

    # Parse the API response
    data = response.json()
    active_tvl = data.get("data", {}).get("active_tvl")

    if active_tvl is None:
        log.error("Failed to retrieve active_tvl from API response")
        return
...
def lambda_handler(event, context):
    ...
    for token in tokens:
        try:
            supply = get_total_supply(token['slug'], babylon_rpc_url)
