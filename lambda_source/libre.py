...
# Fetch total supply
def fetch_total_supply(url, token_name):
    try:
        payload = {
            "code": "btc.ptokens",
            "symbol": token_name
        ...
        if response.status_code != 200:
            log.error(f"Failed to retrieve data: {response.status_code}")
            return
        
        data = response.json()
        if token_name not in data:
            log.error(f"{token_name} not found in the response.")
            return

        total_supply = data[token_name].get("supply", None)
        if not total_supply:
            log.error(f"{token_name} supply not found in the response.")
            return

        total_supply_float = float(total_supply.split(' ')[0])
        return total_supply_float
...
def lambda_handler(event, context):
    ...
    for token in tokens:
        try:
            token_name = token['slug'].split('-')[1].split('_')[0]
